...
class TrailParticle(pygame.sprite.Sprite):
  ...
  def update_graphics(self):
    ...
    colour = self.colour
    colour.a = int(round(255 * (1 - linear(self.age, constants.POWERUP_PARTICLE_AGE))))
...
class PowerupParticle(pygame.sprite.Sprite):
  ...
  def update_graphics(self):
    ...
    self.image.fill(c)
    actual_position = self.position.translate(normal * amp)
    self.rect = self.image.get_rect(center=actual_position.scale(constants.TILE_SIZE))
